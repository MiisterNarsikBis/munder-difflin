  ...
  const openHive = async (path: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setError(res.error ?? t('hivePicker.couldNotOpen', 'Could not open that folder.'));
        setBusy(undefined);
      }
    } catch (e) {
      window.localStorage.removeItem(SKIP_KEY);
          ...
          <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6, maxHeight: 220, overflowY: 'auto' }}>
                  {recents.map((h) => (
                    <button
                      ...
                      onClick={() => openHive(h)}
                      disabled={!!busy}
                      title={t('hivePicker.switchToTooltip', 'Switch to {path} (reloads the app)', { path: h })}
                      style={{
                    ...
                    >
                      ...
                      <span style={{ fontSize: 11, color: 'var(--cth-ink-500)', flexShrink: 0 }}>
                        {busy === h ? t('hivePicker.opening', 'opening…') : t('hivePicker.switchArrow', 'switch →')}
                      </span>
                    </button>
                  ))}
                </div>
            ...
            {busy && (
              <div style={{ fontSize: 12, color: 'var(--cth-ink-500)' }}>
                {t('hivePicker.openingReload', 'Opening {name} — the app will reload…', { name: folderName(busy) })}
              </div>
            )}
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', flexWrap: 'wrap' }}>
              <PixelButton variant="secondary" size="md" onClick={browse} disabled={!!busy}>
                <span style={{ display: 'inline-flex', gap: 6, alignItems: 'center' }}>
                  <Icon name="folder" /> {t('hivePicker.openExisting', 'open existing config…')}
                </span>
              </PixelButton>
              <PixelButton variant="secondary" size="md" onClick={browse} disabled={!!busy}>
                <span style={{ display: 'inline-flex', gap: 6, alignItems: 'center' }}>
                  <Icon name="plus" /> {t('hivePicker.createNew', 'create new config…')}
                </span>
              </PixelButton>
            </div>
          </div>
