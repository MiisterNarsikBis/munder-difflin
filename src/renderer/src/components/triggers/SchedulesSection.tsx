...
function relTime(ms: number): string {
  const past = ms >= 0;
  const a = Math.abs(ms);
  if (a < 45_000) return translate('schedulesSection.justNow', 'just now');
  const mins = Math.round(a / 60_000);
  const unit = mins < 60 ? `${mins}` + translate('schedulesSection.unit.m', 'm')
    : mins < 1440 ? `${Math.round(mins / 60)}` + translate('schedulesSection.unit.h', 'h')
    : `${Math.round(mins / 1440)}` + translate('schedulesSection.unit.d', 'd');
  return past ? translate('schedulesSection.unitAgo', '{unit} ago', { unit }) : translate('schedulesSection.inUnit', 'in {unit}', { unit });
}
...
export function SchedulesSection({ onSummary }: { onSummary?: (s: string) => void }) {
  ...
  useEffect(() => {
    ...
    onSummary?.(missions.length === 0 ? t('schedulesSection.none', 'none') : t('schedulesSection.nOfTotalOn', '{on} of {total} on', { on, total: missions.length }));
  ...
  const targetName = (to: string) =>
    to === 'broadcast' ? t('schedulesSection.everyone', 'everyone') : to === 'god' ? 'Michael' : agents.find((a) => a.id === to)?.name ?? to;

  return (
    <>
      {missions.length === 0 && <Muted>{t('schedulesSection.nothingScheduled', 'Nothing is scheduled yet.')}</Muted>}
      {missions.map((m) => (
        <MissionRow
          key={m.id}
          mission={m}
      ...
      {!adding && (
        <div style={{ marginTop: 8 }}>
          <PixelButton variant="secondary" size="sm" onClick={() => setAdding(true)}>{t('schedulesSection.addASchedule', 'add a schedule')}</PixelButton>
        </div>
      )}
      {adding && (
        <SubCard>
          <div style={{ fontFamily: 'var(--cth-font-display)', fontSize: 8, color: 'var(--cth-ink-500)' }}>{t('schedulesSection.newSchedule', 'NEW SCHEDULE')}</div>
          <Field label={t('schedulesSection.label', 'LABEL')}>
            <input
              value={mLabel}
              onChange={(e) => setMLabel(e.target.value)}
              placeholder={t('schedulesSection.whatThisRunIsFor', 'What this run is for')}
              style={inputStyle}
            />
          </Field>
          <Field label={t('schedulesSection.goesTo', 'GOES TO')}>
            <Select value={mTo} onChange={setMTo} style={{ width: '100%' }}>
              <option value="broadcast">{t('schedulesSection.everyone', 'everyone')}</option>
              <option value="god">Michael</option>
              {agents.filter((a) => !a.isGod).map((a) => <option key={a.id} value={a.id}>{a.name}</option>)}
            </Select>
          </Field>
          <Field label={t('schedulesSection.every', 'EVERY')}>
            <IntervalPicker value={mInterval} onChange={setMInterval} />
          </Field>
          <Field label={t('schedulesSection.prompt', 'PROMPT')}>
            <textarea
              value={mBody}
              onChange={(e) => setMBody(e.target.value)}
              rows={3}
              placeholder={t('schedulesSection.sentWordForWord', 'Sent word for word on every run.')}
              style={textareaStyle}
            />
          </Field>
          <div style={{ display: 'flex', gap: 6, marginTop: 8 }}>
            <PixelButton variant="primary" size="sm" onClick={add} disabled={!mLabel.trim() || !mBody.trim()}>
              {t('schedulesSection.add', 'add')}
            </PixelButton>
            <PixelButton variant="ghost" size="sm" onClick={() => { setAdding(false); setMLabel(''); setMBody(''); }}>
              {t('schedulesSection.cancel', 'cancel')}
            </PixelButton>
          </div>
        </SubCard>
      )}
...
function MissionRow({ mission, targetName, agents, onPatch, onDelete }: {
  ...
  const fired = mission.lastFiredAt
    ? t('schedulesSection.fired', 'fired {age}', { age: relTime(Date.now() - mission.lastFiredAt) })
    : t('schedulesSection.notYetFired', 'not yet fired');
  const next = mission.enabled && mission.lastFiredAt
    ? ' · ' + t('schedulesSection.next', 'next {age}', { age: relTime(Date.now() - (mission.lastFiredAt + mission.intervalMs)) })
    : '';
  ...
  return (
    <SubCard>
        ...
        title={
          <span style={{ display: 'flex', alignItems: 'center', gap: 6, minWidth: 0 }}>
            <Chip tone={mission.enabled ? 'on' : 'off'}>
              {heartbeat ? '♥ ' + t('schedulesSection.beat', 'beat') : fmtInterval(mission.intervalMs)}
            </Chip>
      ...
      {!open && (
        ...
        }}>{mission.body.trim() || t('schedulesSection.noPromptSet', 'No prompt set.')}</div>
      )}
      ...
      {open && (
        <div style={{ marginTop: 4 }}>
          ...
          <Field label={t('schedulesSection.every', 'EVERY')}>
            ...
            {heartbeat && <Hint>{t('schedulesSection.beatHint', 'The beat adapts to how quiet the floor is, so this is the ceiling, not the exact gap.')}</Hint>}
          </Field>
          <Field label={t('schedulesSection.prompt', 'PROMPT')}>
            <textarea
              value={body}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 8 }}>
            <PixelButton variant="primary" size="sm" onClick={save} disabled={!dirty || !label.trim()}>
              {saved && !dirty ? t('schedulesSection.saved', 'saved') : t('schedulesSection.save', 'save')}
            </PixelButton>
            <span style={{ flex: 1 }} />
            <MiniButton tone="danger" onClick={onDelete}>{t('schedulesSection.delete', 'delete')}</MiniButton>
          </div>
        </div>
      )}
    </SubCard>
