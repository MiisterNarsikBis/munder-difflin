...
function statusLabel(code: string): string {
  return code === 'M' ? translate('gitTab.status.modified', 'modified')
    : code === 'A' ? translate('gitTab.status.added', 'added')
    : code === 'D' ? translate('gitTab.status.deleted', 'deleted')
    : code === 'R' ? translate('gitTab.status.renamed', 'renamed')
    : code === '?' ? translate('gitTab.status.untracked', 'untracked')
    : code === 'U' ? translate('gitTab.status.unmerged', 'unmerged')
    : code === ' ' ? '' : code;
}
  ...
  if (isRepo === false) {
    return (
      <div style={{
        ...
        {t('gitTab.notARepo', 'Not a git repo.')}<br />{t('gitTab.runGitInit', "Run")} <code style={{ fontFamily: 'var(--cth-font-mono)' }}>git init</code> {t('gitTab.inAgentsTerminal', "in the agent's terminal.")}
      </div>
    );
  }
    ...
    <div style={{
      ...
      <div style={{
        ...
        <span style={{
          ...
          {detached ? t('gitTab.detachedHead', 'DETACHED HEAD') : (branch ?? '—')}
        </span>
        {upstream && (
        ...
        <div style={{ marginLeft: 'auto' }}>
          <PixelButton variant="ghost" size="sm" onClick={refresh} disabled={loading}>
            {loading ? '...' : t('gitTab.refresh', 'refresh')}
          </PixelButton>
        </div>
      </div>
      ...
      <div style={{ flex: 1, minHeight: 0, overflow: 'auto' }}>
        {/* Status */}
        <Section title={t('gitTab.section.status', 'status')}>
          {status && (
            <>
              <StatusGroup label={t('gitTab.group.staged', 'staged')} entries={status.staged.map(e => ({ ...e, code: e.index }))} />
              <StatusGroup label={t('gitTab.group.changes', 'changes')} entries={status.unstaged.map(e => ({ ...e, code: e.worktree }))} />
              <StatusGroup label={t('gitTab.group.untracked', 'untracked')} entries={status.untracked.map(p => ({ path: p, code: '?' }))} />
              {status.staged.length === 0 && status.unstaged.length === 0 && status.untracked.length === 0 && (
                <div style={{
                  padding: '4px 12px', color: 'var(--cth-ink-500)', fontSize: 13
                }}>{t('gitTab.workingTreeClean', 'working tree clean')}</div>
              )}
            </>
          )}
        </Section>
        ...
        {branches && (branches.local.length > 0 || branches.remote.length > 0) && (
          <Section title={t('gitTab.section.branches', 'branches')}>
        ...
        {/* Graph */}
        <Section title={t('gitTab.section.log', 'log')}>
          {log.length > 0 ? <CommitGraph commits={log} currentBranch={branch} /> : (
            <div style={{ padding: 12, color: 'var(--cth-ink-500)', fontSize: 12 }}>{t('gitTab.noCommitsYet', 'no commits yet')}</div>
          )}
        </Section>
      </div>
    </div>
      ...
      {entries.map(e => (
        <div key={`${label}-${e.path}-${e.code}`} style={{
          ...
          <button
            ...
            title={t('gitTab.copyPath', 'Copy path')}
            style={{
