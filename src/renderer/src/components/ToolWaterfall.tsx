        ...
        {sample ? (
          <>
            ...
            <span style={{ color: 'var(--cth-ink-700)' }}>
              {t('toolWaterfall.fresh', 'fresh {n}t', { n: fmtTokens(sample.input + sample.cacheCreation) })}
            </span>
            <span style={{ color: 'var(--cth-sky)' }}>
              {t('toolWaterfall.cache', 'cache {n}t ({pct}%)', { n: fmtTokens(sample.cacheRead), pct: Math.round(cacheFraction(sample) * 100) })}
            </span>
            {sample.model && <span style={{ color: 'var(--cth-ink-500)' }}>{sample.model}</span>}
            <span style={{ color: 'var(--cth-ink-500)' }}>{t('toolWaterfall.total', '{n}t total', { n: fmtTokens(totalTokens(sample)) })}</span>
          </>
        ) : (
          <span style={{ color: 'var(--cth-ink-500)' }}>{t('toolWaterfall.noTelemetry', 'no live telemetry yet — spawn / respawn this agent to instrument it')}</span>
        )}
      </div>
      ...
      <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: 10 }}>
        {recent.length === 0 && (
          <div style={{ fontSize: 12, color: 'var(--cth-ink-500)' }}>
            {t('toolWaterfall.noToolCalls', 'No tool calls captured yet. Each tool the agent runs appears here with its real duration.')}
          </div>
        )}
        {recent.map((s, i) => {
          ...
          return (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 3 }}>
              ...
              <div style={{ flex: 1, height: 12, background: 'var(--cth-paper-100)', boxShadow: 'inset 0 0 0 1px var(--cth-ink-300)' }}>
                <div
                  title={s.error ? `${s.tool}: ${s.error}` : `${s.tool} · ${s.durationMs}ms · ${ok ? t('toolWaterfall.ok', 'ok') : t('toolWaterfall.failed', 'failed')}`}
