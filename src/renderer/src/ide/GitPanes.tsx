...
export function HistoryPane({ gitRoot, onOpenRevDiff }: {
  ...
  const jump = async (c: GitCommitRow) => {
    if (!window.confirm(t('gitPanes.jumpConfirm', 'Jump the repo to {sha} ("{subject}")?\n\nThis detaches HEAD. Blocked automatically if the tree is dirty or an agent is mid-run.', { sha: c.shortSha, subject: c.subject.slice(0, 60) }))) return;
    const res = await window.cth.gitCheckout(gitRoot, c.sha, true);
    setNote(res.ok ? t('gitPanes.nowAtDetached', 'now at {sha} (detached HEAD)', { sha: c.shortSha }) : res.error);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: 0, flex: 1 }}>
      <div style={{ flex: 1, minHeight: 0, overflow: 'auto' }}>
        {loading && commits.length === 0 && <div style={noteStyle}>{t('gitPanes.loadingHistory', 'loading history…')}</div>}
        {!loading && commits.length === 0 && <div style={noteStyle}>{t('gitPanes.noCommits', 'no commits')}</div>}
        <CommitGraph commits={commits} currentBranch={branch} onCommitClick={(sha) => { void pick(sha); }} />
        {commits.length >= page * 200 && (
          <div style={{ padding: '4px 12px' }}>
            <button style={smallBtn} onClick={() => setPage((p) => p + 1)}>{t('gitPanes.loadOlder', 'load older…')}</button>
          </div>
        )}
      </div>
      {selected && (
        <div style={{
          ...
          <div style={{
            ...
            <button style={smallBtn} onClick={() => void jump(selected)} title={t('gitPanes.checkoutThisCommit', 'Check out this commit (detached HEAD)')}>
              <Icon name="arrow-right" /> {t('gitPanes.jumpHere', 'jump here')}
            </button>
            <button style={{ ...smallBtn, width: 20, justifyContent: 'center' }} onClick={() => setSelected(null)} title={t('gitPanes.close', 'Close')}>✕</button>
          </div>
          ...
          <div style={{ flex: 1, overflow: 'auto', minHeight: 0 }}>
            {!files && !note && <div style={noteStyle}>{t('gitPanes.loadingFiles', 'loading files…')}</div>}
            {note && <div style={{ ...noteStyle, color: 'var(--cth-ink-700)' }}>{note}</div>}
            {files && files.length === 0 && <div style={noteStyle}>{t('gitPanes.noFileChanges', 'no file changes (merge?)')}</div>}
            {files?.map((f) => (
              <FileRow
                key={f.path}
                f={f}
...
export function ComparePane({ gitRoot, onOpenRevDiff }: {
  ...
  useEffect(() => {
    ...
    let alive = true;
    setNote(t('gitPanes.comparing', 'comparing…'));
  ...
  const switchTo = async () => {
    if (!head) return;
    if (!window.confirm(t('gitPanes.switchConfirm', "Switch this repo to '{head}'?\n\nBlocked automatically if the tree is dirty or an agent is mid-run.", { head }))) return;
    const res = await window.cth.gitCheckout(gitRoot, head.replace(/^origin\//, ''), false);
    setNote(res.ok ? t('gitPanes.switchedTo', 'switched to {head}', { head }) : res.error);
  };
    ...
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: 0, flex: 1 }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 4, padding: '6px 12px', flexShrink: 0 }}>
        <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
          <select value={base} onChange={(e) => setBase(e.target.value)} style={sel} title={t('gitPanes.baseTitle', "Base — the branch you're comparing against")}>
            {branches.map((b) => <option key={b} value={b}>{b}</option>)}
          </select>
          <button style={{ ...smallBtn, width: 22, justifyContent: 'center' }} title={t('gitPanes.swap', 'Swap base ↔ compare')}
            onClick={() => { setBase(head); setHead(base); }}>⇄</button>
          <select value={head} onChange={(e) => setHead(e.target.value)} style={sel} title={t('gitPanes.compareTitle', "Compare — the branch whose changes you're viewing")}>
            {branches.map((b) => <option key={b} value={b}>{b}</option>)}
          </select>
        </div>
        <div style={{ display: 'flex', gap: 6, alignItems: 'center', fontSize: 11, color: 'var(--cth-ink-500)' }}>
          {result && (
            <span title={t('gitPanes.aheadBehindTitle', "'{head}' is {ahead} ahead and {behind} behind '{base}'", { head, ahead: result.ahead, behind: result.behind, base })}>
          ...
          <button
            ...
            title={mode === 'three'
              ? t('gitPanes.modeThreeTitle', 'Showing what the compare branch ADDS since the common ancestor (PR-style). Click for the literal two-dot difference.')
              : t('gitPanes.modeTwoTitle', 'Showing the literal difference between the two branch states. Click for PR-style (what compare adds).')}
          >{mode === 'three' ? t('gitPanes.sinceCommonAncestor', 'since common ancestor') : t('gitPanes.literalDifference', 'literal difference')}</button>
          <span style={{ flex: 1 }} />
          <button style={smallBtn} onClick={() => void switchTo()} title={t('gitPanes.checkout', "Check out '{head}'", { head })}>
            <Icon name="arrow-right" /> {t('gitPanes.switchTo', 'switch to {branch}', { branch: head.split('/').pop() ?? head })}
          </button>
        </div>
      </div>
      <div style={{ flex: 1, minHeight: 0, overflow: 'auto', borderTop: '1px solid var(--cth-ink-100)' }}>
        {note && <div style={noteStyle}>{note}</div>}
        {result && result.files.length === 0 && !note && <div style={noteStyle}>{t('gitPanes.noDifferences', 'no differences')}</div>}
        {result?.files.map((f) => (
          <FileRow
            key={f.path}
            f={f}
